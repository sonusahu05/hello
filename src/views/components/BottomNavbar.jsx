        ...
        <>
            <footer className="bg-white shadow-up py-4 text-gray-800">
                <div className="container mx-auto flex flex-col md:flex-row lg:flex-row justify-between items-start ">
                    ...
                    <div className="lg:flex-auto lg:w-2/12 mb-6 mx-4">
                        <h1 className="mb-2 text-red_color text-3xl cursor-pointer" onClick={() => toggleDropdown('AllProjects')}>
                            Projects
                        </h1>
                        {Object.entries(projectsData).map(([category, projects]) => (
                            <div key={category}>
                                <button className="flex  items-center justify-between  hover:text-red_color mb-2" onClick={() => toggleDropdown(category)}>
                                    {category}
                                    {expanded[category] ? <FaAngleUp /> : <FaAngleDown />}
                                </button>
                                {expanded[category] && (
                                    <ul className='pl-3'>
                                        {projects.map((project, index) => (
                                            <li key={index}>
                                                <a href="/projects" className="text-gray-500 hover:text-red_color">{project}</a>
                                            </li>
                                        ))}
                                    </ul>
                                )}
